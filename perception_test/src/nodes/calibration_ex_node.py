...
class ExtrinsicCalibrationManager:
    def __init__(self):
        ...
        self.min_track_len = 20        # 최소 15프레임 이상 추적된 궤적만 사용

        # [핵심] 정지 차량 배제
        self.min_speed_mps = 4

        self.cluster_tol = 5.0       # 레이더 클러스터링 거리 임계값
        self.match_max_dist_px = 1000.0  # 초기 매칭 허용 범위 (전역 매칭)

        # RANSAC 설정
        self.ransac_iterations = 1000
        self.ransac_threshold_deg = 0.3 # 2도 이내 오차만 인라이어 판정
