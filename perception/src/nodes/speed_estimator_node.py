...
데이터 흐름:
[Association Node] (/perception/associated)
       ⬇
[Speed Estimator] (Gating -> Bias -> KF)
       ⬇
[Final Output] (/perception/associated_speed)
"""
...
class SpeedEstimatorNode:
    def __init__(self):
        ...
        # --- 파라미터 설정 ---
        # 입력/출력 토픽
        self.sub_topic = rospy.get_param("~in_topic", "/perception/associated")
        self.pub_topic = rospy.get_param("~out_topic", "/perception/associated_speed")
        ...
        self.max_speed_kph = float(rospy.get_param("~gate/max_speed_kph", 200.0))
    ...
    def callback(self, msg: AssociationArray):
        ...
        for obj in msg.objects:
            ...
            if np.isnan(raw_speed) or abs(raw_speed) > self.max_speed_kph:
                is_valid = False
